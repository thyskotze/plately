...
- **Goals = a calorie *window* + protein**; carbs & fat are auto-derived (`deriveMacros`, fat ≈30% of kcal). `Goals.kcalMin`/`kcalMax` hold a dietitian-style range (e.g. 2300–2500) and `kcal` is its mid-point (drives the ring target and macro derivation). Both range fields are **optional** — older goals fall back to a band around `kcal` via `kcalWindow()`, so no migration was needed. **`scoreDay(totals, goals)` in `calc.ts` is the single source of truth for what a good day is**: `onTarget` means inside the window; **going over counts against you exactly like falling short** (this is a weight-loss app, not a target to maximise). `proteinHit` is a bonus only — it never gates the streak. Streak, Stats badges, the Home ring/achievement card and the consumption summary all read from `scoreDay`; don't reintroduce a bare `>= goals.kcal` check anywhere. Protein is treated as a floor (beating it shows green), carbs/fat as ceilings.
...
- **Consumption summary** (`src/components/ConsumptionSummary.tsx`, on Stats): per-day eaten kcal + P/C/F over either the last 7 rolling days or the current calendar week (local toggle), plus daily averages. Averages **only count days with something eaten**, so a mid-week view isn't dragged down by days that haven't happened yet.
- **Serving sizes**: `Food.servings?: {label,grams}[]` (optional) comes from an AI import (7th `|` column, parsed by `parseServings`) or a scanned product's label/pack size (Open Food Facts `serving_quantity`/`quantity`). `frequentPortions(mealsByDay, foodId)` derives habitual amounts from meal history — no extra stored state. `GramsSheet` offers both as one-tap chips and `chooseFood` pre-fills the first serving (or most-used amount) instead of a flat 100 g.
- **Barcode from the meal picker**: `openBarcodeScan(true)` sets ephemeral `barcodeToSlot`, so after a successful lookup `BarcodeSheet` calls `chooseFood` instead of closing — landing on the portion step with the pending day/slot intact.
- **Coach report (PDF)** — Stats → *Coach report*. `src/lib/report.ts` builds `ReportData` for the last 7/14/30 days (eaten meals only, scored with `scoreDay`; today counts only once it's in range or over, same as the streak) and draws two A4 canvases (charts + daily log). `src/lib/pdf.ts` is a tiny dependency-free PDF writer that embeds each canvas as a full-page JPEG, so text in the PDF isn't selectable. Sharing uses the Web Share API with a PDF file, falling back to a download. `WeightEntry.date` (optional) is now stamped on new weigh-ins; the report's weight trend only uses dated entries.
- **App version** is baked from `package.json` as `__APP_VERSION__` (see `vite.config.ts`) and shown in Profile's footer with the build date. Bump `package.json` `version` on notable releases.
...
- **Work it out for me** — Goals sheet / Profile / a Stats nudge (shown when the latest weigh-in is 2 kg+ off `bio.weight`, i.e. the weight goals were set at). `src/lib/targets.ts`: Mifflin–St Jeor × activity, deficit/surplus from a pace in kg/week (7,700 kcal/kg; cut capped at 25% of burn, floored at max(BMR, 1500♂/1200♀)), protein 1.8 g/kg (1.6 maintain) on an *adjusted* weight above BMI 25, fat 30%, carbs remainder, fibre/water shown only. `measureBurn` estimates real TDEE from eaten kcal + a least-squares weight slope (needs dated weigh-ins spanning 14+ days and 10+ logged days) and can replace the formula. Apply writes `goals`, `bio` (now with optional `pace`) and `weightGoal`.
- **Usual foods / pre-fill** — `src/lib/routine.ts` detects items logged in the same slot on ≥50% of the last 28 days (≥3 times; weekday/weekend-only items detected separately; on days where meals are ticked only ticked slots count). `prefillDay` fills **empty slots only**, today or later, once per day (`routine.filled`), on app open/foreground and when a Calendar day is selected. Pre-filled portions carry `auto: true` (a "usual" tag); editing grams drops it. Profile → *Your usual foods* shows detections, per-item toggles (`routine.off`) and the master switch. `routine` is persisted (missing on old saves → defaults via zustand merge).
## Constraints & gotchas
